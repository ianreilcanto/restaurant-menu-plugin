...
$secColPercentage = .84;
$height = isset($_GET['rowHeight']) ? $_GET['rowHeight'] : 60;
$defaultHeight = 5;

$pageWidth = $_GET['width'];
...
{  
 ...
 if($img){
    ...
    if($img && $isRender)
    {
            ...
            $imageYpo = $tableYPos + $contentPadding;
    ...
    $pdf->MultiCell($firstColWidth, $defaultHeight,"\n".'Tel: '.$restaurants_value[$i]['telephone']."\n".$restaurants_value[$i]['web_url'], 0, 'C', 0, 0, $offset, $contactContainerPosY - 10, true);

    // //content
    $pdf->MultiCell($secColWidth - $offset, $defaultHeight,$restaurants_value[$i]["general_text"], 0, 'L', 0, 0,($offset + $firstColWidth) + $contentPadding, $tableYPos + $height - $offset -10 + 4, true);
          ...
          //assign value
            foreach($weekValues as $key => $value)
            {
                for( $l = 0; count($menu_value[$j]['menu_list']) > $l; $l++){
                    ...
                    if( ( $nowWeekNum + 1 ) == ( $menuDateWeekNumber ) && $key == $weekDay['weekday'])
                    {
                        //$key == $weekDay['weekday'] && $nowWeekDay['month'] == $weekDay['month'] && $nowWeekNumber == $weekNumber
        ...
        $specialChar = array( 
           ...
           foreach($weekValues as $key => $value)
           {
                ...
                }else
                {
                    ...
                    $cellHeight = $contentHeight - $contentPadding - 7;
